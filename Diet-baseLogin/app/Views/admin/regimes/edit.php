...
<body>
    ...
    <div class="container">
        ...
        <form method="post" action="/admin/regimes/update/<?= $regime['id'] ?>">
            ...
            <input type="text" name="nom" value="<?= old('nom', $regime['nom']) ?>" required>
            
            <label>Description</label>
            <textarea name="description" rows="3"><?= old('description', $regime['description']) ?></textarea>
            
            <label>Pourcentage viande (%) *</label>
            <input type="number" name="pourcentage_viande" value="<?= old('pourcentage_viande', $regime['pourcentage_viande']) ?>" min="0" max="100" required>
            
            <label>Pourcentage poisson (%) *</label>
            <input type="number" name="pourcentage_poisson" value="<?= old('pourcentage_poisson', $regime['pourcentage_poisson']) ?>" min="0" max="100" required>
            
            <label>Pourcentage volaille (%) *</label>
            <input type="number" name="pourcentage_volaille" value="<?= old('pourcentage_volaille', $regime['pourcentage_volaille']) ?>" min="0" max="100" required>
            
            <label>Effet sur le poids par semaine (kg) *</label>
            <input type="number" step="0.1" name="effet_poids_par_semaine" value="<?= old('effet_poids_par_semaine', $regime['effet_poids_par_semaine']) ?>" required>
            <small>Négatif = perte de poids | Positif = gain de poids</small>
            
            <label>Prix par jour (€) *</label>
            <input type="number" step="0.01" name="prix_par_jour" value="<?= old('prix_par_jour', $regime['prix_par_jour']) ?>" required>
